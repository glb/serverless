...
module.exports = {
  compileRestApi() {
    ...
    if (!_.isEmpty(this.serverless.service.provider.resourcePolicy)) {
      // The documentation clearly states that `resourcePolicy` should be the list
      // of statements in the resource policy, but if someone pastes in
      // their actual desired resource policy object instead, this avoids a nasty
      // accidental nesting that will be difficult to troubleshoot.
      const policy = Array.isArray(this.serverless.service.provider.resourcePolicy)
        ? {
            Version: '2012-10-17',
