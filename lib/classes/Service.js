  ...
  mergeArrays() {
    ['resources', 'functions'].forEach((key) => {
      ...
      if (Array.isArray(this[key])) {
        this[key] = this[key].reduce((memo, value, index) => {
          if (value) {
            if (typeof value === 'object') {
              // Look for duplicate resource IDs, as merging these could cause problems.
              if (typeof value.Resources === 'object' && !Array.isArray(value.Resources)) {
                Object.entries(value.Resources).forEach(([k]) => {
                  const wasSeenIn = resourceIDs[k];

                  if (wasSeenIn !== undefined) {
                    const message = `Duplicate resource logical ID \`${k}\` found in resources! Original item was in ${key}[${wasSeenIn}], duplicate found in ${key}[${index}].`;
                    switch (this.provider.duplicateResourceBehavior) {
                      case 'warn':
                        logWarning(message);
                        break;
                      case 'error':
